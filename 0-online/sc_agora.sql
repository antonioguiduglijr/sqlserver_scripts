/***
Descrição: Monitora status das conexões no banco de dados - SQL 2005 / SQL 2008
Autor....: Antonio Guidugli Junior
Data.....: 22/01/2014
***/

EXEC master..sp_WhoIsActive @sort_order = '[start_time] ASC', @show_sleeping_spids = 0,@output_column_list = '[dd%][session_id][block%][sql_text][database_name][login_name][hostname][program_name][wait_info][cpu%][temp%][reads%][writes%][physical%][%]' -- [sql_command][tasks][tran_log%][context%][query_plan][locks]
--EXEC master..sp_WhoIsActive @help = 1
...
SELECT a.session_id,
       ...
       --CONVERT(VARCHAR(2),DATEDIFF(HH,a.start_time,GETDATE()))+':'+CONVERT(VARCHAR(2),DATEDIFF(MI,a.start_time,GETDATE()))+':'+CONVERT(VARCHAR(2),DATEDIFF(SS,a.start_time,GETDATE())) as Run, --TEMPO DE EXECUÇÃO EM MINUTOS
...
SELECT c.session_id, c.properties, c.creation_time, c.is_open, t.text
  FROM sys.dm_exec_cursors (79) c --SPID
 CROSS 
...
SELECT DISTINCT	
       ...
       DATEDIFF ( MI, A.LAST_BATCH, GETDATE() ) AS RUN_MIN, --TEMPO DE EXECUÇÃO EM MINUTOS
       DATEDIFF ( SS, A.LAST_BATCH, GETDATE() ) AS RUN_SEG, --TEMPO DE EXECUÇÃO EM MINUTOS
